...
import de.bund.bfr.knime.openkrise.db.gui.dbtable.editoren.MyMNSQLJoinCollector;
...
public class MyTable {
	...
	public String getRowCountSQL() {
		return "SELECT COUNT(*) FROM " + MyDBI.delimitL(tableName);
	}

	public String getSelectSQL() {
		String fieldDefs = MyDBI.delimitL("ID");
		for (int i = 0; i < fieldNames.length; i++) {
			fieldDefs += "," + MyDBI.delimitL(fieldNames[i]);
		}
		if (!hideScore) fieldDefs += "," + MyDBI.delimitL("Guetescore");
		if (!hideComment) fieldDefs += "," + MyDBI.delimitL("Kommentar");
		if (!hideTested) fieldDefs += "," + MyDBI.delimitL("Geprueft");
		return "SELECT " + fieldDefs + " FROM " + MyDBI.delimitL(tableName);
	}

	public String getInsertSQL1() {
		return "INSERT INTO " + MyDBI.delimitL(tableName) + " " + getInsertSql();
	}
	...
	public String getUpdateSQL1() {
		return "UPDATE " + MyDBI.delimitL(tableName) + " SET " + getUpdateSql();
	}
	...
	public String getDeleteSQL1() {
		return "DELETE FROM " + MyDBI.delimitL(tableName) + " WHERE " + MyDBI.delimitL("ID") + " = ?";
	}
		...
		for (int i = 0; i < fieldNames.length; i++) {
			if (foreignFields[i] != null) {
				if (mnTable == null || mnTable[i] == null || mnTable[i].length() == 0) {
					indexSQL.add("ALTER TABLE " + MyDBI.delimitL(tableName) + " ADD CONSTRAINT " + MyDBI.delimitL(tableName + "_fk_" + fieldNames[i] + "_" + i)
							+ " FOREIGN KEY (" + MyDBI.delimitL(fieldNames[i]) + ")" + " REFERENCES " + MyDBI.delimitL(foreignFields[i].getTablename()) + " ("
							+ MyDBI.delimitL("ID") + ") " + (odsn ? "ON DELETE SET NULL;" : ";"));
				}
			}
		}
		if (uniqueFields != null) {
			for (int i = 0; i < uniqueFields.length; i++) {
				...
				for (int j = 0; j < uniqueFields[i].length; j++) {
					uFs += "," + MyDBI.delimitL(uniqueFields[i][j]);
				}
				indexSQL.add("ALTER TABLE " + MyDBI.delimitL(tableName) + " ADD CONSTRAINT " + MyDBI.delimitL(tableName + "_uni_" + i) + " UNIQUE (" + uFs.substring(1)
						+ ");");
			}
		}
		return indexSQL;
	...
	public void createTable(Connection conn) {
		String fieldDefs = MyDBI.delimitL("ID") + " INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY";
		if (tableName.equals("ChangeLog")) {
			fieldDefs = MyDBI.delimitL("ID") + " INTEGER GENERATED BY DEFAULT AS SEQUENCE " + MyDBI.delimitL("ChangeLogSEQ") + " PRIMARY KEY";
		}
		//String uFsAll = "";
		for (int i = 0; i < fieldNames.length; i++) {
			if (fieldTypes[i].startsWith("BLOB(") && !tableName.equals("DateiSpeicher")) {
				fieldDefs += "," + MyDBI.delimitL(fieldNames[i]) + " " + "VARCHAR(255)";
			} else {
				String defolt = null;
				if (defaults != null && defaults[i] != null && !fieldTypes[i].startsWith("BLOB(")) defolt = defaults[i];
				fieldDefs += "," + MyDBI.delimitL(fieldNames[i]) + " " + fieldTypes[i] + (defolt == null ? "" : " " + defolt);
				if (!tableName.equals("DateiSpeicher") && !tableName.equals("ChangeLog")) {
					//uFsAll += "," + MyDBI.delimitL(fieldNames[i]);
				}
			}
		}
		if (!hideScore) fieldDefs += "," + MyDBI.delimitL("Guetescore") + " " + "INTEGER";
		if (!hideComment) fieldDefs += "," + MyDBI.delimitL("Kommentar") + " " + "VARCHAR(1023)";
		if (!hideTested) fieldDefs += "," + MyDBI.delimitL("Geprueft") + " " + "BOOLEAN";
		//createTable(tableName, fieldDefs, getIndexSQL(), true, true);

		try {
			if (tableName.equals("ChangeLog")) {
				if (existsDBKernel()) {
					DBKernel.sendRequest(conn, "CREATE SEQUENCE " + MyDBI.delimitL(tableName + "SEQ") + " AS INTEGER START WITH 1 INCREMENT BY 1", false, false);
					DBKernel.sendRequest(conn, "GRANT USAGE ON SEQUENCE " + MyDBI.delimitL("ChangeLogSEQ") + " TO " + MyDBI.delimitL("PUBLIC"), false, false);
				}
			}

			Statement stmt = conn == null ? DBKernel.getDBConnection().createStatement() : conn.createStatement();
			String sqlc = "CREATE CACHED TABLE " + MyDBI.delimitL(tableName) + " (" + fieldDefs + ");";
			stmt.execute(sqlc);
			...
			if (existsDBKernel()) {
				if (!tableName.equals("ChangeLog") && !tableName.equals("DateiSpeicher") && !tableName.equals("Infotabelle")) {
					stmt.execute("CREATE TRIGGER " + MyDBI.delimitL("A_" + tableName + "_U") + " AFTER UPDATE ON " + MyDBI.delimitL(tableName) + " FOR EACH ROW " + " CALL "
							+ MyDBI.delimitL(new MyTrigger().getClass().getName()));
					stmt.execute("CREATE TRIGGER " + MyDBI.delimitL("A_" + tableName + "_D") + " AFTER DELETE ON " + MyDBI.delimitL(tableName) + " FOR EACH ROW " + " CALL "
							+ MyDBI.delimitL(new MyTrigger().getClass().getName()));
					stmt.execute("CREATE TRIGGER " + MyDBI.delimitL("A_" + tableName + "_I") + " AFTER INSERT ON " + MyDBI.delimitL(tableName) + " FOR EACH ROW " + " CALL "
							+ MyDBI.delimitL(new MyTrigger().getClass().getName()));
				}
			}
			stmt.close();
	...
	private String getUpdateSql() {
		String result = "";
		for (int i = 0; i < fieldNames.length; i++) {
			result += MyDBI.delimitL(fieldNames[i]) + "=?,";
		}
		if (!hideScore) result += MyDBI.delimitL("Guetescore") + "=?,";
		if (!hideComment) result += MyDBI.delimitL("Kommentar") + "=?,";
		if (!hideTested) result += MyDBI.delimitL("Geprueft") + "=?,";
		if (result.length() > 0) result = result.substring(0, result.length() - 1); // letztes Komma weg!
		result += " WHERE " + MyDBI.delimitL("ID") + "=?";
		return result;
	}
	...
	private String getInsertSql() {
		...
		for (int i = 0; i < fieldNames.length; i++) {
			...
			result += MyDBI.delimitL(columnName) + ",";
			qms += "?,";
		}
		if (!hideScore) {
			result += MyDBI.delimitL("Guetescore") + ",";
			qms += "?,";
		}
		if (!hideComment) {
			result += MyDBI.delimitL("Kommentar") + ",";
			qms += "?,";
		}
		if (!hideTested) {
			result += MyDBI.delimitL("Geprueft") + ",";
			qms += "?,";
		}
		if (result.length() > 0) result = result.substring(0, result.length() - 1); // letztes Komma weg!
			...
			for (String s : this.getFields2ViewInGui()) {
				...
				if (mt2 == null) {
					...
					if (!mnsqlc.getAlreadyJoined().contains(mt2)) {
						String join = " LEFT JOIN " + MyDBI.delimitL(mt2.getTablename()) + " ON " + MyDBI.delimitL(this.getTablename()) + "." + MyDBI.delimitL(s) + "="
								+ MyDBI.delimitL(mt2.getTablename()) + "." + MyDBI.delimitL("ID");
	...
	private String getAdd2Select(String fieldName, Integer fi) {
		...
		String field = MyDBI.delimitL(this.getTablename()) + "." + MyDBI.delimitL(fieldName);
	...
	private String getSQL(MyTable myFT, String myMN) {
		...
		String toSelect = MyDBI.delimitL(myFT.getTablename()) + "." + MyDBI.delimitL("ID");
		...
		if (mnT != null) {
			...
			if (fn != null) {
				toWhere = " WHERE " + MyDBI.delimitL(mnT.getTablename()) + "." + MyDBI.delimitL(fn) + "=";
			} else {
			...
			if (fn != null) {
				toWhere = " WHERE " + MyDBI.delimitL(myFT.getTablename()) + "." + MyDBI.delimitL(fn) + "=";
			} else {
				System.err.println("mnF2 = null...\t" + myFT + "\t" + this);
			}
		}
		sql = "SELECT " + mnsqlc.getToSelect() + " FROM " + MyDBI.delimitL(myFT.getTablename()) + mnsqlc.getToJoin() + toWhere;
		return sql;
	}
	...
	private String getMNJoin(MyTable mnT) {
		...
		if (mnT != null) {
			...
			if (mnF1 != null) {
				toJoin += " LEFT JOIN " + MyDBI.delimitL(mnT.getTablename()) + " ON " + MyDBI.delimitL(mnT.getTablename()) + "." + MyDBI.delimitL(mnF1) + "="
						+ MyDBI.delimitL(this.getTablename()) + "." + MyDBI.delimitL("ID");
			} else {
		...
		if (listMNs != null) {
			...
			for (int i = 0; i < listMNs.size(); i++) {
				...
				DBKernel.sendRequest(
						"UPDATE " + MyDBI.delimitL(tableName) + " SET " + MyDBI.delimitL(feldname) + "=" + MyDBI.delimitL("ID") + " WHERE " + MyDBI.delimitL(feldname)
								+ " IS NULL OR " + MyDBI.delimitL(feldname) + "!=" + MyDBI.delimitL("ID"), true);
			}
		}
		DBKernel.dontLog = dl;
