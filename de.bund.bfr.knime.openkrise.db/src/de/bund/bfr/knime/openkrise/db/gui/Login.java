...
import de.bund.bfr.knime.openkrise.db.UpdateChecker;
import de.bund.bfr.knime.openkrise.db.VersionComprator;
	...
	private boolean doTheUpdates() {
		...
		try {
			...
			if (DBKernel.getDBVersionFromDB().equals("1.8.3")) {
				UpdateChecker.check4Updates_183_184();
				DBKernel.setDBVersion("1.8.4");
			}
			...
			if (DBKernel.getDBVersionFromDB().equals("1.8.6")) {
				UpdateChecker.check4Updates_186_187();
				DBKernel.setDBVersion("1.8.7");
			}
			DBKernel.closeDBConnections(false);
		} catch (Exception e) {
			e.printStackTrace();
