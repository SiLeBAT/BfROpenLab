...
public abstract class MultiDimensionViewDialog extends ViewDialog {

	protected String lastSelectedID;
	protected String lastVarX;

	protected MultiDimensionViewDialog() {
		lastSelectedID = null;
		lastVarX = null;
	}
	...
	protected ViewSettings createSettings() {
		return new MultiDimensionViewSettings();
	}
	...
	protected void initChartComponents() {
		configPanel = new ChartConfigPanel(ChartConfigPanel.InputType.VARIABLE_BOXES, allowInterval());
		selectionPanel = reader.getSelectionPanelBuilder().build();
		chartCreator = new ChartCreator(reader.getPlotables(), reader.getLegend());

		Plotable plotable = reader.getPlotables().get(((MultiDimensionViewSettings) set).getSelectedID());
