...
public class MyKrisenInterfacesNodeModel extends NodeModel {
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData,
		...
		if (randomGen) {
			RandomNetworkGenerator rng = new RandomNetworkGenerator(randomGenNodes,
					randomGenLinking);
			...
			if (rsp != null && rsp.first()) {
				do {
					warningMessage = "Dates correct?? In: " + rsp.getString("ID_In") + " ("
							+ rsp.getInt("Day_In") + "." + rsp.getInt("Month_In") + "."
							+ rsp.getInt("Year_In") + ") vs. Out: " + rsp.getString("ID_Out")
							+ " (" + rsp.getInt("Day_Out") + "." + rsp.getInt("Month_Out") + "."
							+ rsp.getInt("Year_Out") + ")";
					System.err.println(warningMessage);
				...
				do {
					if (rsp.getObject("Amount_In") != null && rsp.getObject("Amount_Out") != null) {
						...
						if (in > out * 2 || out > in * 2) { // 1.1
							warningMessage = "Amounts correct?? In: " + rsp.getString("ids_in")
									+ " (" + in + ") vs. Out: " + rsp.getString("ids_out") + " ("
									+ out + ")";
							System.err.println(warningMessage);
							this.setWarningMessage(warningMessage);
							warningsThere = true;
			...
			Map<String, Delivery> allDeliveries = MyNewTracingLoader.getNewTracingModel(
					DBKernel.myDBi, conn);
			...
			ResultSet rs = DBKernel.getResultSet(conn,
					"SELECT * FROM " + DBKernel.delimitL("Station"), false);
			if (rs != null && rs.first()) {
				int rowNumber = 0;
				...
				do {
					...
					if (rowNumber == 0 && bll != null
							&& (bll.equals("Altenburger Land") || bll.equals("Wesel")))
						isBVLFormat = true;
					// if (!antiArticle ||
					...
					String bl = getBL(bll);
					String company = (rs.getObject("Name") == null || doAnonymize) ? getAnonymizedStation(
							bl, sID.hashCode(), country) : clean(rs.getString("Name")); // bl
																						// +
																						// stationID
																						// +
																						// "("
					...
					fillCell(
							spec,
							cells,
							TracingColumns.STATION_STREET,
							doAnonymize ? DataType.getMissingCell() : getDataStringCell(rs,
									"Strasse"));
					fillCell(
							spec,
							cells,
							TracingColumns.STATION_HOUSENO,
							doAnonymize ? DataType.getMissingCell() : getDataStringCell(rs,
									"Hausnummer"));
					...
					fillCell(spec, cells, TracingColumns.STATION_DISTRICT, doAnonymize
							|| district == null ? DataType.getMissingCell() : new StringCell(
							district));
					fillCell(spec, cells, TracingColumns.STATION_STATE,
							doAnonymize || bll == null ? DataType.getMissingCell()
									: new StringCell(bll));
					fillCell(spec, cells, TracingColumns.STATION_COUNTRY, doAnonymize
							|| country == null ? DataType.getMissingCell()
							: new StringCell(country));

					fillCell(
							spec,
							cells,
							TracingColumns.STATION_VAT,
							doAnonymize ? DataType.getMissingCell() : getDataStringCell(rs,
									"VATnumber"));
					fillCell(spec, cells, TracingColumns.STATION_TOB,
							getDataStringCell(rs, "Betriebsart"));
					fillCell(spec, cells, TracingColumns.STATION_NUMCASES,
							getDataIntCell(rs, "AnzahlFaelle"));
					fillCell(spec, cells, TracingColumns.STATION_DATESTART,
							getDataStringCell(rs, "DatumBeginn"));
					fillCell(spec, cells, TracingColumns.STATION_DATEPEAK,
							getDataStringCell(rs, "DatumHoehepunkt"));
					fillCell(spec, cells, TracingColumns.STATION_DATEEND,
							getDataStringCell(rs, "DatumEnde"));
					fillCell(spec, cells, TracingColumns.STATION_SERIAL,
							getDataStringCell(rs, "Serial"));
					fillCell(spec, cells, TracingColumns.STATION_SIMPLESUPPLIER,
							isSimpleSupplier(allDeliveries, sID) ? BooleanCell.TRUE
									: BooleanCell.FALSE);
					fillCell(spec, cells, TracingColumns.STATION_DEADSTART,
							isStationStart(allDeliveries, sID) ? BooleanCell.TRUE
									: BooleanCell.FALSE);
					...
					fillCell(spec, cells, TracingColumns.FILESOURCES,
							getDataStringCell(rs, "ImportSources"));
						...
						if (extraCol.startsWith("_")) {
							...
							ResultSet rs2 = DBKernel.getResultSet(
									...
									"SELECT " + DBKernel.delimitL("value") + " FROM "
											+ DBKernel.delimitL("ExtraFields") + " WHERE "
											+ DBKernel.delimitL("tablename") + "='Station' AND "
			...
			rs = DBKernel.getResultSet(
					conn,
					"SELECT * FROM " + DBKernel.delimitL("Lieferungen") + " LEFT JOIN "
			...
			if (rs != null && rs.first()) {
				...
				do {
					...
					String lieferID = rs.getObject(useSerialAsID ? "Lieferungen.Serial" : lID)
							.toString();
					...
					fillCell(spec, cells, TracingColumns.DELIVERY_ITEMNAME,
							getDataStringCell(rs, "Bezeichnung"));
					fillCell(
							spec,
							cells,
							TracingColumns.DELIVERY_ITEMNUM,
							doAnonymize ? DataType.getMissingCell() : getDataStringCell(rs,
									"Artikelnummer"));
					...
					String ad = sdfFormat(clean(rs.getString("Lieferungen.ad_day")),
							clean(rs.getString("Lieferungen.ad_month")),
							clean(rs.getString("Lieferungen.ad_year")));
					...
					fillCell(spec, cells, TracingColumns.DELIVERY_PROCESSING,
							getDataStringCell(rs, "Prozessierung"));
					fillCell(spec, cells, TracingColumns.DELIVERY_USAGE,
							getDataStringCell(rs, "IntendedUse"));
					fillCell(
							spec,
							cells,
							...
							doAnonymize ? DataType.getMissingCell() : getDataStringCell(rs,
									"ChargenNr"));
					String mhd = sdfFormat(clean(rs.getString("MHD_day")),
							clean(rs.getString("MHD_month")), clean(rs.getString("MHD_year")));
					fillCell(spec, cells, TracingColumns.DELIVERY_DATEEXP,
							mhd == null ? DataType.getMissingCell() : new StringCell(mhd));
					String pd = sdfFormat(clean(rs.getString("pd_day")),
							clean(rs.getString("pd_month")), clean(rs.getString("pd_year")));
					...
					fillCell(spec, cells, TracingColumns.DELIVERY_AMOUNT,
							menge == null ? DataType.getMissingCell() : new DoubleCell(
									menge / 1000.0));
					fillCell(spec, cells, TracingColumns.DELIVERY_NUM_PU,
							getDataDoubleCell(rs, "Lieferungen.numPU"));
					fillCell(spec, cells, TracingColumns.DELIVERY_TYPE_PU,
							getDataStringCell(rs, "Lieferungen.typePU"));
					...
					fillCell(
							spec,
							cells,
							TracingColumns.DELIVERY_CHARGENUM,
							doAnonymize ? DataType.getMissingCell() : getDataStringCell(rs,
									"ChargenNr"));
					...
					for (String extraCol : spec.getColumnNames()) {
						if (extraCol.startsWith("_")) {
							...
							ResultSet rs2 = DBKernel.getResultSet(
									...
									"SELECT " + DBKernel.delimitL("value") + " FROM "
											+ DBKernel.delimitL("ExtraFields") + " WHERE "
											+ DBKernel.delimitL("tablename") + "='" + tn + "' AND "
				...
				for (String next : delivery.getAllNextIds()) {
					if (useSerialAsID)
						deliveryDelivery.addRowToTable(new DefaultRow(i + "", IO
								...
								.createCell(hmDeliveryIDs.get(next))));
					else
						deliveryDelivery.addRowToTable(new DefaultRow(i + "", IO
								.createCell(delivery.getId()), IO.createCell(next)));
					i++;
				}
			}
		}
	...
	private DataCell getDataIntCell(ResultSet rs, String columnname) throws SQLException {
		return rs.getObject(columnname) == null ? DataType.getMissingCell() : new IntCell(
				rs.getInt(columnname));
	}

	private DataCell getDataDoubleCell(ResultSet rs, String columnname) throws SQLException {
		return rs.getObject(columnname) == null ? DataType.getMissingCell() : new DoubleCell(
	...
	private String sdfFormat(String day, String month, String year) {
		...
		if (year != null && year.length() == 2)
			year = Integer.parseInt(year) > Integer.parseInt(thisYear.substring(2)) ? "19" : "20"
					+ year;
	...
	private DataTableSpec getSpec33Nodes(Connection conn) {
		...
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_NODE, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_NAME, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_STREET, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_HOUSENO, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_ZIP, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_CITY, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_DISTRICT, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_STATE, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_COUNTRY, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_SERIAL, StringCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_SIMPLESUPPLIER,
				BooleanCell.TYPE).createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_DEADSTART, BooleanCell.TYPE)
				.createSpec());
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_DEADEND, BooleanCell.TYPE)
				.createSpec());

		if (containsValues(conn, "Station", "VATnumber"))
			columns.add(new DataColumnSpecCreator(TracingColumns.STATION_VAT, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Station", "Betriebsart"))
			columns.add(new DataColumnSpecCreator(TracingColumns.STATION_TOB, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Station", "AnzahlFaelle"))
			columns.add(new DataColumnSpecCreator(TracingColumns.STATION_NUMCASES, IntCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Station", "DatumBeginn"))
			columns.add(new DataColumnSpecCreator(TracingColumns.STATION_DATESTART, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Station", "DatumHoehepunkt"))
			columns.add(new DataColumnSpecCreator(TracingColumns.STATION_DATEPEAK, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Station", "DatumEnde"))
			columns.add(new DataColumnSpecCreator(TracingColumns.STATION_DATEEND, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Station", "ImportSources"))
			columns.add(new DataColumnSpecCreator(TracingColumns.FILESOURCES, StringCell.TYPE)
					.createSpec());

		// due to backward compatibility:
		columns.add(new DataColumnSpecCreator(TracingColumns.STATION_COUNTY, StringCell.TYPE)
				.createSpec());
		...
		try {
			ResultSet rs = DBKernel.getResultSet(
					conn,
					"SELECT DISTINCT(" + DBKernel.delimitL("attribute") + ") FROM "
							+ DBKernel.delimitL("ExtraFields") + " WHERE "
							+ DBKernel.delimitL("tablename") + "='Station'", false);
			if (rs != null && rs.first()) {
				do {
					columns.add(new DataColumnSpecCreator("_" + rs.getString(1), StringCell.TYPE)
							.createSpec());
				} while (rs.next());
			}
		} catch (Exception e) {
	...
	private DataTableSpec getSpec33Links(Connection conn) {
		...
		columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_ITEMNUM, StringCell.TYPE)
				.createSpec());
		...
		if (containsValues(conn, "Produktkatalog", "Prozessierung"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_PROCESSING,
					StringCell.TYPE).createSpec());
		if (containsValues(conn, "Produktkatalog", "IntendedUse"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_USAGE, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Chargen", "ChargenNr"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_LOTNUM, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Chargen", new String[] { "MHD_day", "MHD_month", "MHD_year" }))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_DATEEXP, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Chargen", new String[] { "pd_day", "pd_month", "pd_year" }))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_DATEMANU, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Lieferungen", "Unitmenge"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_AMOUNT, DoubleCell.TYPE)
					.createSpec());
		boolean acv = containsValues(conn, "Lieferungen", "numPU");
		if (acv)
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_NUM_PU, DoubleCell.TYPE)
					.createSpec());
		if (acv && containsValues(conn, "Lieferungen", "typePU"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_TYPE_PU, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Chargen", "OriginCountry"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_ORIGIN, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Lieferungen", "EndChain"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_ENDCHAIN, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Lieferungen", "Explanation_EndChain"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_ENDCHAINWHY,
					StringCell.TYPE).createSpec());
		if (containsValues(conn, "Lieferungen", "Contact_Questions_Remarks"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_REMARKS, StringCell.TYPE)
					.createSpec());
		if (containsValues(conn, "Lieferungen", "Further_Traceback"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_FURTHERTB,
					StringCell.TYPE).createSpec());
		if (containsValues(conn, "Chargen", "MicrobioSample"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_MICROSAMPLE,
					StringCell.TYPE).createSpec());
		...
		if (containsValues(conn, "Chargen", "ChargenNr"))
			columns.add(new DataColumnSpecCreator(TracingColumns.DELIVERY_CHARGENUM,
					StringCell.TYPE).createSpec());
		...
		try {
			ResultSet rs = DBKernel.getResultSet(
					...
					"SELECT DISTINCT(CONCAT(" + DBKernel.delimitL("tablename") + ",'.',"
							+ DBKernel.delimitL("attribute") + ")) FROM "
							+ DBKernel.delimitL("ExtraFields") + " WHERE "
							+ DBKernel.delimitL("tablename") + "='Produktkatalog'" + " OR "
							+ DBKernel.delimitL("tablename") + "='Chargen'" + " OR "
							+ DBKernel.delimitL("tablename") + "='Lieferungen'", false);
			if (rs != null && rs.first()) {
				do {
					columns.add(new DataColumnSpecCreator("_" + rs.getString(1), StringCell.TYPE)
							.createSpec());
	...
	private boolean containsValues(Connection conn, String tablename, String[] columnnames) {
		...
		if (conn != null) {
			try {
				...
				ResultSet rs = DBKernel.getResultSet(conn, "SELECT (" + count + ") FROM "
						+ DBKernel.delimitL(tablename), false);
		...
		try {
			if (rs != null && rs.first()) {
				do {
					...
					sql = "UPDATE " + DBKernel.delimitL("Station") + " SET "
							+ DBKernel.delimitL("Name") + "='" + anonStr + "', "
							+ DBKernel.delimitL("Strasse") + "=NULL, "
							+ DBKernel.delimitL("Hausnummer") + "=NULL, "
							+ DBKernel.delimitL("Ort") + "=NULL WHERE " + DBKernel.delimitL("ID")
							+ "=" + rs.getInt("ID");
					DBKernel.sendRequest(conn, sql, false, false);
					sql = "UPDATE " + DBKernel.delimitL("Station") + " SET "
							+ DBKernel.delimitL("Betriebsnummer") + "=NULL WHERE "
	...
	private static Connection getNewLocalConnection(final String dbUsername,
